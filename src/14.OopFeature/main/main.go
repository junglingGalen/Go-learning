...
二、继承
	...
	② 结构体的属性查找，先从自身属性查找，找不到会去自己继承的匿名结构体查找（所以pupil.Student.Score可以写成pupil.Score）
	如果都找不到则报错
	③ 当结构体和匿名结构体有相同字段或者方法时，编译器采用就近访问原则访问，如果希望访问
	匿名结构体的字段和方法，可以通过匿名结构体名来区分
	④ 多重继承： 结构体嵌入两个（或多个）匿名结构体，如两个匿名结构体有相同的字段和方法（同时结构体本身没有同名字段和方法）
	在访问时，就必须明确指定匿名结构体类型，否则编译器报错（建议不使用多重继承）
